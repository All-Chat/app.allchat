...
async function generateCountsData(userId: string, cacheKey: string, lockKey: string) {
  try {
    ...
    const campaigns = await Campaign.aggregate([
      ...
      { $sort: { createdAt: -1 } },
      { $project: { name: 1, templateName: 1, templateCategory: 1, variables: 1, mappedVariables: 1, generateOtp: 1, otpLength: 1, phoneNumbers: { $slice: [{ $ifNull: ["$phoneNumbers", []] }, 15] }, names: { $slice: [{ $ifNull: ["$names", []] }, 15] }, additionalFieldsData: { $slice: [{ $ifNull: ["$additionalFieldsData", []] }, 15] }, mediaUrl: 1, mediaType: 1, languageCode: 1, status: 1, totalMessages: 1, totalDeducted: 1, scheduledAt: 1, createdAt: 1, additionalFields: 1, sentCount: 1, failedCount: 1, skippedCount: 1, liveStats: { total: { $ifNull: ["$totalMessages", 0] }, replied: { $size: { $filter: { input: { $ifNull: ["$reportData", []] }, as: "r", cond: { $or: [ { $ne: [ { $ifNull: ["$$r.reply", ""] }, "" ] }, { $gt: [ { $size: { $filter: { input: { $ifNull: ["$$r.replies", []] }, as: "rep", cond: { $ne: ["$$rep", ""] } } } }, 0 ] } ] } } }, read: { $size: { $filter: { input: { $ifNull: ["$reportData", []] }, as: "r", cond: { $eq: [{ $toLower: { $ifNull: ["$$r.status", ""] } }, "read"] } } } }, delivered: { $size: { $filter: { input: { $ifNull: ["$reportData", []] }, as: "r", cond: { $eq: [{ $toLower: { $ifNull: ["$$r.status", ""] } }, "delivered"] } } } }, invalid: { $size: { $filter: { input: { $ifNull: ["$reportData", []] }, as: "r", cond: { $eq: [{ $toLower: { $ifNull: ["$$r.status", ""] } }, "invalid"] } } } }, duplicate: { $size: { $filter: { input: { $ifNull: ["$reportData", []] }, as: "r", cond: { $eq: [{ $toLower: { $ifNull: ["$$r.status", ""] } }, "duplicate"] } } } } } } }
  }]);
    ...
    const fixedCampaigns = campaigns.map((c: any) => {
      ...
      return { ...c, liveStats: { ...ls, sent, failed, pending, deliveredRead: delivered + read, failedInvalid: failed + invalid, progress }, languageCode: c.languageCode || "en", totalDeducted: c.totalDeducted || 0 };
    });

    const redisClient = (await new (require('bullmq').Queue)('counts-processing', { connection }).client);
    await redisClient.set(cacheKey, JSON.stringify({ campaigns: fixedCampaigns, _cachedAt: Date.now() }), 'EX', 3600);
...
async function refreshReportCache(data: any) {
  ...
  try {
    ...
    const redisClient = (await new (require('bullmq').Queue)('report-processing', { connection }).client);
...
const campaignWorker = new Worker('campaign-processing', async (job) => {
  if (job.name === 'send-chunk') await processCampaignChunk(job.data);
}, { 
  connection,
  concurrency: 1,
...
const countsWorker = new Worker('counts-processing', async (job) => {
  if (job.name === 'generate-counts') return await generateCountsData(job.data.userId, job.data.cacheKey, job.data.lockKey);
}, { connection, concurrency: 5 });

const reportWorker = new Worker('report-processing', async (job) => {
  if (job.name === 'refresh-report-cache') return await refreshReportCache(job.data);
}, { connection, concurrency: 5 });
...
countsWorker.on('completed', () => console.log(`✅ Counts job completed`));
countsWorker.on('failed', (job: any, err: Error) => console.error(`❌ Counts job ${job?.id ?? ''} failed:`, err.message));
...
console.log('🚀 Standalone BullMQ Workers started in background...');
