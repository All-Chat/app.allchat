...
connectDB().then(async () => {
  console.log('✅ Sheet Worker connected to MongoDB');
  startSheetCampaignWorker();
});
...
async function processSheetSync(config: any) {
  try {
    ...
    console.log(`[Sheet Worker] Auto-fetching sheet data for: ${config.name || config._id}`);
    ...
    console.log(`[Sheet Worker] Successfully fetched ${rows.length} rows for: ${config.name}`);
    ...
    console.error(`[Sheet Worker] Error auto-fetching sheet ${config._id}:`, error.message);
    try {
      config.lastSynced = new Date();
...
async function processSheetCampaign(campaign: any) {
  try {
    ...
    console.log(`[Sheet Worker] Processing sheet campaign: ${campaign.name}`);
    ...
    console.error(`[Sheet Worker] Error processing sheet campaign ${campaign._id}:`, error.message);
  }
}

async function startSheetCampaignWorker() {
  console.log("🚀 Worker started for: Google Sheet Campaigns (Interval: 10s)");
  while (true) {
    try {
      ...
      if (activeCampaigns.length > 0) {
        await Promise.allSettled(activeCampaigns.map(c => processSheetCampaign(c)));
      }

    } catch (error) {
      console.error("[Sheet Worker] Loop error:", error);
    } finally {
...
process.on('SIGTERM', async () => {
  console.log('Sheet Worker process shutting down...');
  process.exit(0);
});
