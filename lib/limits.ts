...
const LIMIT_RESOURCES = ["tags", "workflows", "templates", "testMessages", "campaigns", "optNumbers", "forms", "whatsappNumbers"] as const;
...
export async function checkLimit(userId: string, resource: LimitResource): Promise<LimitCheckResult> {
  ...
  if (!user) return { allowed: false, remaining: 0, limit: 0, period: "unlimited", currentUsage: 0, message: "User not found" };
  ...
  if (limit.period === "unlimited" || limit.max === -1) {
    return { allowed: true, remaining: Infinity, limit: -1, period: "unlimited", currentUsage: usage.count || 0 };
  }

  if (usage.resetAt && new Date() > new Date(usage.resetAt)) {
    (user as any).usage[resource] = { count: 0, resetAt: getNextResetDate(limit.period) };
    await user.save();
    usage.count = 0;
  }
  ...
  return {
    allowed, remaining, limit: maxAllowed, period: limit.period, currentUsage: currentCount,
    message: allowed ? undefined : `Limit reached: ${currentCount}/${maxAllowed} ${resource} per ${limit.period}`,
  };
}

export async function incrementUsage(userId: string, resource: LimitResource, count: number = 1): Promise<void> {
  await connectDB();
  const user = await User.findById(userId);
  if (!user) return;
...
export async function decrementUsage(userId: string, resource: LimitResource, count: number = 1): Promise<void> {
  await connectDB();
  const user = await User.findById(userId);
  if (!user) return;
  const usage = (user as any).usage?.[resource];
...
export async function getUserLimitsAndUsage(userId: string) {
  ...
  for (const resource of LIMIT_RESOURCES) {
    ...
    result[resource] = {
      limit, usage: { count: u.count || 0, resetAt: u.resetAt },
