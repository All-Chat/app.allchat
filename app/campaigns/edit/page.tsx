...
function EditCampaignContent() {
      ...
      if (data.success) {
        ...
        if (campaign) {
          ...
          if (campaign.scheduledAt)
            setScheduleDate(new Date(campaign.scheduledAt).toISOString().slice(0, 16));
        ...
        {mediaInputType === "url" ? (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 focus:bg-white transition-all text-sm font-medium shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"
            />
          </div>
        ) : (
          <div>
            {!mediaFile ? (
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer bg-slate-50 hover:bg-indigo-50/30 hover:border-indigo-300 transition-all">
                ...
                <input
                  type="file"
                  className="hidden"
                  accept={mediaType === "image" ? "image/*" : mediaType === "video" ? "video/*" : ".pdf"}
  ...
  const handleSave = async (isSchedule: boolean) => {
    if (!campaignName || !selectedTemplate || rawNumbers.length === 0) {
      toast.error("Name, Template, and valid Numbers are required");
      return;
    }
    ...
    if (mediaType && mediaInputType === "upload" && !mediaFile && !mediaUrl) {
                    ...
                    {variables.map((v, i) => (
                      <div key={i} className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-20 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 focus:bg-white transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"
                        />
                      </div>
                    ))}
                  </div>
                      ...
                      <div className="bg-white rounded-xl rounded-tr-sm shadow-sm text-sm text-gray-800 leading-relaxed max-w-[95%] overflow-hidden">
                        {mediaType === "image" && (
                          <div className="w-full bg-slate-100">
                            {previewMediaSrc ? (
                              <img src={previewMediaSrc} alt="Preview" className="w-full h-48 object-cover" />
                            ) : (
                        ...
                        {mediaType === "video" && (
                          <div className="w-full bg-slate-900 h-48 flex items-center justify-center relative">
                            {previewMediaSrc ? (
                              <video src={previewMediaSrc} className="w-full h-48 object-cover" muted />
                            ) : (
                          ...
                          <div className="w-full bg-slate-100 p-4 flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-bold text-slate-800 truncate">
                                {mediaFile ? mediaFile.name : previewMediaSrc ? previewMediaSrc.split("/").pop() : "Document.pdf"}
                              </p>
              ...
              <div className="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 space-y-5 sm:space-y-6 hover:shadow-md transition-shadow">
                ...
                {uploadStep === 1 ? (
                  <div className="relative border-2 border-dashed border-slate-200 rounded-2xl p-6 sm:p-10 text-center hover:bg-indigo-50/30 hover:border-indigo-300 transition-all h-48 sm:h-56 flex flex-col items-center justify-center group cursor-pointer">
                    ...
                    />
                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-indigo-50 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-indigo-100 group-hover:scale-110 transition-all duration-300 shadow-sm">
                  ...
                  <div className="border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white rounded-2xl p-4 sm:p-6 space-y-4 sm:space-y-5 shadow-sm">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-indigo-100 shadow-sm min-w-0">
                    ...
                    <button
                      ...
                      className="w-full px-5 py-3.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl text-sm font-bold hover:from-indigo-600 hover:to-purple-600 transition-all shadow-md flex items-center justify-center gap-2"
                    >
                ...
                <div className="relative">
                  <textarea
                    ...
                    placeholder="Or manually paste numbers..."
                    className="w-full pl-5 pr-24 sm:pr-28 py-4 bg-slate-50 border border-slate-200 rounded-xl resize-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 focus:bg-white transition-all h-32 text-sm font-mono shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"
                  />
                  <button
                    onClick={handleTextNumbers}
                    className="absolute right-3 bottom-3 px-4 sm:px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg text-xs font-bold hover:from-emerald-600 hover:to-teal-600 transition-all shadow-md flex items-center gap-1.5"
                  >
                    <Send size={12} /> Parse
                  </button>
                </div>
                ...
                {rawNumbers.length > 0 && (
                  <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center pt-2">
                    <div className="bg-gradient-to-br from-emerald-50 to-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-emerald-100 shadow-sm">
                      <p className="text-2xl sm:text-3xl font-extrabold text-emerald-600">{stats.valid}</p>
                    ...
                    <div className="bg-gradient-to-br from-red-50 to-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-red-100 shadow-sm">
                      <p className="text-2xl sm:text-3xl font-extrabold text-red-600">{stats.invalid}</p>
                    ...
                    <div className="bg-gradient-to-br from-amber-50 to-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-amber-100 shadow-sm">
                      <p className="text-2xl sm:text-3xl font-extrabold text-amber-600">{stats.duplicates}</p>
                ...
                <div className="flex flex-col gap-6">
                  <div className="w-full">
                    <label className="text-[11px] font-extrabold text-slate-800 flex items-center gap-2 uppercase tracking-widest mb-2">
                      <Clock size={14} className="text-indigo-500" /> Reschedule Campaign (Optional)
                    </label>
                    <input
                      type="datetime-local"
                      value={scheduleDate}
                      ...
                      className="w-full px-4 sm:px-5 py-3 sm:py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 focus:bg-white transition-all text-sm font-medium shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"
                    />
                    <p className="text-[10px] text-amber-600 bg-amber-50 p-2 rounded-lg border border-amber-100 flex items-center gap-1.5 mt-2 font-bold">
                      <AlertCircle size={10} /> Must be at least 15 mins in advance.
                    </p>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      ...
                      disabled={saving}
                      className="flex-1 sm:flex-none px-6 sm:px-8 py-3 sm:py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-bold hover:from-emerald-600 hover:to-teal-600 flex items-center justify-center gap-2 text-sm transition-all shadow-md disabled:opacity-40"
                    >
...
// ✅ DEFAULT EXPORT: wraps the component in a Suspense boundary
