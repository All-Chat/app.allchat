    ...
    return (
      <div className="space-y-3">
        <div className="flex gap-2 bg-slate-100 p-1 rounded-lg">
          <button type="button" onClick={() => { setMediaInputType("url"); clearMediaFile(); }} className={`flex-1 flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-md text-xs font-bold transition-all ${mediaInputType === "url" ? "bg-white shadow-sm text-emerald-700" : "text-slate-500 hover:text-slate-700"}`}><Link size={14} /> URL</button>
          <button type="button" onClick={() => { setMediaInputType("upload"); setMediaUrl(""); }} className={`flex-1 flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-md text-xs font-bold transition-all ${mediaInputType === "upload" ? "bg-white shadow-sm text-emerald-700" : "text-slate-500 hover:text-slate-700"}`}><Upload size={14} /> Upload</button>
        </div>
        {mediaInputType === "url" ? (
          <div className="relative">
            ...
            <input type="url" value={mediaUrl} onChange={(e) => setMediaUrl(e.target.value)} placeholder={`Direct ${mediaType.charAt(0).toUpperCase() + mediaType.slice(1)} URL`} className="w-full pl-10 pr-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500 focus:bg-white transition-all text-sm font-medium shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]" />
          </div>
        ) : (
          <div>
            {!mediaFile ? (
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer bg-slate-50 hover:bg-emerald-50/30 hover:border-emerald-300 transition-all">
    ...
    try {
      ...
      if (mediaInputType === "upload" && mediaFile) {
        ...
        res = await fetch("/api/campaigns/save", {
          ...
          body: JSON.stringify({
            name: campaignName, templateName: selectedTemplate.name, templateCategory: selectedTemplate.category,
                ...
                {uploadStep === 1 ? (
                  ...
                  <div className="border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white rounded-2xl p-4 sm:p-6 space-y-4 sm:space-y-5 shadow-sm">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-emerald-100 shadow-sm min-w-0"><FileSpreadsheet className="w-5 h-5 text-emerald-600 shrink-0" /><p className="text-sm font-bold text-emerald-900 truncate">{fileName}</p></div>
                    ...
                    <select value={selectedPhoneCol} onChange={(e) => setSelectedPhoneCol(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500 transition-all font-medium shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"><option value="skip">-- Select Phone Column --</option>{fileHeaders.map((h, i) => <option key={i} value={h}>📱 {h}</option>)}</select>
                    <select value={selectedNameCol} onChange={(e) => setSelectedNameCol(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500 transition-all font-medium shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]"><option value="skip">-- Select Name Column (Optional) --</option>{fileHeaders.map((h, i) => <option key={i} value={h}>👤 {h}</option>)}</select>
                    <button onClick={processFileColumns} className="w-full px-5 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl text-sm font-bold hover:from-emerald-600 hover:to-teal-600 transition-all shadow-md flex items-center justify-center gap-2"><Sparkles size={16} /> Extract Audience</button>
                  </div>
                )}

                <div className="relative">
                  <textarea value={rawText} onChange={(e) => setRawText(e.target.value)} placeholder="Or manually paste numbers (comma or newline separated)..." className="w-full pl-5 pr-24 sm:pr-28 py-4 bg-slate-50 border border-slate-200 rounded-xl resize-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500 focus:bg-white transition-all h-32 text-sm font-mono shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)]" />
                  <button onClick={handleTextNumbers} className="absolute right-3 bottom-3 px-4 sm:px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg text-xs font-bold hover:from-indigo-600 hover:to-purple-600 transition-all shadow-md flex items-center gap-1.5"><Send size={12} /> Parse</button>
                </div>
              ...
              <div className="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                <div className="flex flex-col gap-6">
                  <div className="w-full">
                    <label className="text-[11px] font-extrabold text-slate-800 flex items-center gap-2 uppercase tracking-widest mb-2"><Clock size={14} className="text-indigo-500" /> Schedule Campaign (Optional)</label>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button onClick={() => handleSave(false)} disabled={saving} className="flex-1 sm:flex-none px-6 sm:px-8 py-3 sm:py-3.5 bg-slate-100 border border-slate-200 rounded-xl font-bold hover:bg-slate-200 flex items-center justify-center gap-2 text-sm transition-colors text-slate-700 shadow-sm"><FileSpreadsheet size={16} /> Save Draft</button>
                    <button onClick={() => handleSave(true)} disabled={saving || !scheduleDate} className="flex-1 sm:flex-none px-6 sm:px-8 py-3 sm:py-3.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl font-bold hover:from-indigo-600 hover:to-purple-600 flex items-center justify-center gap-2 text-sm disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md"><Clock size={16} /> Schedule</button>
                  </div>
                </div>
              </div>
            </div>
