...
export default function CampaignList() {
  ...
  const loadCampaigns = async () => {
    try {
      // Fetches from the new lightweight counts API
  ...
  const openViewModal = async (c: Campaign) => {
    setLoadingView(true);
    setViewCampaign(c); // Show modal immediately with basic data
    
    try {
      // Fetch full details (including phone numbers) only for this specific campaign
      const res = await fetch(`/api/campaigns/list?id=${c._id}`);
      const data = await res.json();
      
      if (data.success && data.campaigns[0]) {
        setViewCampaign(data.campaigns[0]);
      }
    } catch (err) {
      console.error("Failed to fetch view details", err);
    } finally {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      ...
      // Polls every 5 seconds to update stats without slowing down the page
  ...
  useEffect(() => {
    // Updates the scheduled countdown timers every second
  ...
  const rerunCampaign = async (id: string) => {
    ...
    if (!campaign) { toast.error("Campaign not found."); return; }
  ...
  const quickTestSend = async (c: Campaign) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || data.error?.message || "Failed");
      }
    } catch (err) { 
      toast.error("Error"); 
    }
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setViewCampaign(null)}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-teal-500 p-5 sm:p-6 text-white relative shrink-0">
              <button onClick={() => setViewCampaign(null)} className="absolute top-4 right-4 text-white/80 hover:text-white">
                <X size={20} />
              </button>
            ...
            <div className="p-5 sm:p-6 space-y-5 overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                ...
                <div className="bg-emerald-50 p-2 sm:p-3 rounded-xl text-center border border-emerald-100">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-emerald-600">{viewCampaign.liveStats?.deliveredRead || 0}</p>
                ...
                <div className="bg-red-50 p-2 sm:p-3 rounded-xl text-center border border-red-100">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-red-600">{viewCampaign.liveStats?.failedInvalid || 0}</p>
              ...
              <div className="space-y-3 text-sm border-t border-slate-100 pt-4">
                ...
                {/* Show loading state while fetching phone numbers */}
                {loadingView ? (
        ...
        <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
          ...
          {filteredCampaigns.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {/* Map through current page items */}
              {currentItems.map((c) => {
                const cfg = statusConfig[c.status] || statusConfig.saved;
                
                // ✅ Pull live stats directly from the DB response
                const liveStats = c.liveStats || { 
                  deliveredRead: 0, 
                  sent: 0, 
                  failedInvalid: 0, 
                  pending: 0, 
                  ...
                  <div key={c._id} className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 group">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4">
                      ...
                      <div className="flex items-center gap-1.5 sm:ml-4 w-full sm:w-auto justify-end flex-wrap">
                        <button onClick={() => openViewModal(c)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Details">
                          <Eye size={16} />
                        </button>
                        ...
                        {isCompleted && (
                          <button onClick={() => rerunCampaign(c._id)} disabled={startingId === c._id || !canSendMessage} className={`p-2 rounded-lg transition-colors ${!canSendMessage ? "text-slate-300 cursor-not-allowed" : "text-slate-400 hover:text-purple-600 hover:bg-purple-50"}`} title="Rerun">
                    ...
                    {/* ✅ Perfect Stats Grid */}
                    {/* Mapped directly from the DB counts sent by the API */}
                    <div className={`grid gap-2 sm:gap-3 text-center grid-cols-2 sm:grid-cols-3 md:grid-cols-5 ${amountSpent > 0 ? 'lg:grid-cols-6' : ''}`}>
                      ...
                      <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                        ...
                        <p className="font-bold text-slate-900 text-sm mt-0.5">{liveStats.total}</p>
                      </div>
                      ...
                      <div className="bg-amber-50 p-2 rounded-xl border border-amber-100">
                        ...
                        <p className="font-bold text-amber-700 text-sm mt-0.5">{liveStats.pending}</p>
                      </div>

                      {/* Amount Spent (Only shows if > 0) */}
                      {amountSpent > 0 && (
