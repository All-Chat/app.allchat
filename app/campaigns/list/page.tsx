    ...
    try {
      ...
      if (data.success) {
        ...
        if (action === "resume") {
          ...
          } catch (e) { console.error("Failed to trigger resume start", e); }
        }
      } else {
  ...
  const quickTestSend = async (c: Campaign) => {
    ...
    try {
      ...
      if (c.generateOtp && c.templateCategory === "AUTHENTICATION") {
        ...
        const min = Math.pow(10, len - 1); const max = Math.pow(10, len) - 1;
      ...
      if (res.status === 402) {
        const data402 = await res.json();
        toast.error(data402.message || "Insufficient balance.");
        setCanSendMessage(false); fetchBilling(); return;
      }
      const data = await res.json();
      if (data.success) { toast.success("Test sent!"); fetchBilling(); } else toast.error(data.message || data.error?.message || "Failed");
            ...
            <div className="p-5 sm:p-6 space-y-5 overflow-y-auto">
              ...
              <div className="space-y-3 text-sm border-t border-slate-100 pt-4">
                ...
                <div>
                  ...
                  <div className="bg-slate-50 p-2 rounded-lg text-xs font-mono max-h-20 overflow-y-auto border border-slate-100">
                    ...
                    {viewCampaign.totalMessages > 15 && <span className="text-slate-400">+{viewCampaign.totalMessages - 15} more</span>}
                  </div>
                </div>
              </div>
              <div className="border-t border-slate-100 pt-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button onClick={() => quickTestSend(viewCampaign)} disabled={!canSendMessage} className="px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm font-bold hover:bg-emerald-600 flex items-center justify-center gap-1.5 shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {!canSendMessage && (
            <div className="p-3 sm:p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-600 mt-0.5">You cannot start or rerun campaigns. Please contact your administrator to recharge your account. Go to <a href="/settings" className="underline font-medium">Settings</a> to check your balance.</p>
              </div>
            </div>
          )}
