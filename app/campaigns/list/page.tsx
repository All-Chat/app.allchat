...
/* =========================================================
   COMPUTE STATS
   Delivered = sent + delivered + read + replied
   Failed    = invalid + failed + pending + duplicate
========================================================= */
...
const getCampaignStats = (campaign: Campaign): ComputedStats => {
  ...
  // ✅ Pending is SEPARATE now (not in failed)
  const pendingCount = Number(ls.pending || 0);

  // ✅ Failed = invalid + failed + duplicate (NO pending)
  ...
  return {
    ...
    sent: Number(ls.sent || 0),
    delivered: Number(ls.delivered || 0),
    read: Number(ls.read || 0),
    replied: Number(ls.replied || 0),
    failed: Number(ls.failed || 0),
    invalid: Number(ls.invalid || 0),
    pending: pendingCount,
    duplicate: Number(ls.duplicate || 0),
    deliveredCombined,
    failedCombined,
  };
};
  ...
  const loadCampaigns = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.campaigns)) {
        console.log("CAMPAIGNS FROM DATABASE/API:", data.campaigns);
        data.campaigns.forEach((campaign: Campaign) => {
          console.log("Campaign:", campaign.name, "liveStats:", campaign.liveStats);
        });
          ...
          >
            ...
            <div className="p-5 sm:p-6 space-y-5 overflow-y-auto">
              ...
              {(() => {
                ...
                const amountSpent = stats.deliveredCombined * currentPrice;

                return (
                  <>
                    {/* ═══════════════════════════════════════
                        SUMMARY STATS — Compact: icon + number inline
                    ═══════════════════════════════════════ */}
                    ...
                    {/* AMOUNT SPENT */}
                    {stats.deliveredCombined > 0 && currentPrice > 0 && (
                      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 px-4 py-3 rounded-xl border border-blue-100 flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-slate-400">× {formatINR(currentPrice)} / msg</p>
                        </div>
                      </div>
                    )}

                    {/* ═══════════════════════════════════════
                        BREAKDOWN — Compact inline stat chips
                    ═══════════════════════════════════════ */}
          ...
          {loadingCampaigns ? (
            ...
            <div className="space-y-4">
              ...
              {currentItems.map((c) => {
                ...
                const pendingCount = stats.pending;  // ✅ ADD THIS
                ...
                const amountSpent = deliveredCount * currentPrice;
                ...
                return (
                  ...
                  >
                    ...
                    {/* ═══════════════════════════════════════
                        STATS + PROGRESS
                        ✅ Compact: icon + number inline (horizontal)
                    ═══════════════════════════════════════ */}
                    ...
                    {deliveredCount > 0 && (
                      <div className="mt-3 flex items-center gap-2 text-xs">
                        ...
                        <span className="text-slate-400">
                          ({deliveredCount} delivered × {formatINR(currentPrice)})
                        </span>
                      </div>
                    )}
