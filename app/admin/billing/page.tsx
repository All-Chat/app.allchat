...
{ title: "Bottom Links", links: [ { name: "Transactions", href: "/billing/history" }, { name: "Settings", href: "/settings" } ] },
];
...
orange: { bg: "bg-orange-50", border: "border-orange-200", iconBg: "bg-orange-100", iconText: "text-orange-600", inputBorder: "border-orange-200", inputFocus: "focus/30 focus" },
emerald: { bg: "bg-emerald-50", border: "border-emerald-200", iconBg: "bg-emerald-100", iconText: "text-emerald-600", inputBorder: "border-emerald-200", inputFocus: "focus/30 focus" },
blue: { bg: "bg-blue-50", border: "border-blue-200", iconBg: "bg-blue-100", iconText: "text-blue-600", inputBorder: "border-blue-200", inputFocus: "focus/30 focus" },
violet: { bg: "bg-violet-50", border: "border-violet-200", iconBg: "bg-violet-100", iconText: "text-violet-600", inputBorder: "border-violet-200", inputFocus: "focus/30 focus" },
rose: { bg: "bg-rose-50", border: "border-rose-200", iconBg: "bg-rose-100", iconText: "text-rose-600", inputBorder: "border-rose-200", inputFocus: "focus/30 focus" },
...
const [editTab, setEditTab] = useState<"billing" | "plan" | "account" | "credentials" | "limits" | "tenancy" | "integrations" | "sidebar" | "reports">("billing");
...
const startEdit = (user: any, tab: "billing" | "plan" | "account" | "credentials" | "limits" | "tenancy" | "integrations" | "sidebar" | "reports" = "billing") => {
...
const body: any = { userId, action, ...extraData };
  ...
  if (data.success) {
    ...
    } else if (action !== "integrations" && action !== "sidebar" && action !== "reports") {
      cancelEdit();
    }
...
<input type={showKey ? "text" : "password"} value={adminKey} onChange={e => setAdminKey(e.target.value)} onKeyDown={e => e.key === "Enter" && handleVerify()} placeholder="Enter admin secret key" className="w-full px-4 py-3 sm:py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-gray-900 placeholder-slate-400 focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400 transition-all text-sm" />
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2.5 mb-0.5">
                        ...
                        {user.hiddenReportActions?.length > 0 && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200 flex items-center gap-1 shrink-0"><BarChart3 size={9} /> {user.hiddenReportActions.length} Reports Hidden</span>}
                      </div>
              ...
              {isEditing && (
                <div className="border-t border-slate-100 bg-slate-50/50">
                  <div className="flex overflow-x-auto border-b border-slate-200 px-4 sm:px-6">
                    {[{ id: "billing", label: "Billing", icon: Wallet }, { id: "plan", label: "Plan", icon: CalendarDays }, { id: "account", label: "Account", icon: UserCog }, { id: "credentials", label: "Credentials", icon: KeyRound }, { id: "limits", label: "Limits", icon: Gauge }, { id: "tenancy", label: "Tenancy", icon: Building2 }, { id: "integrations", label: "Integrations", icon: FileText }, { id: "sidebar", label: "Sidebar", icon: LayoutGrid }, { id: "reports", label: "Reports", icon: BarChart3 }].map(tab => (
