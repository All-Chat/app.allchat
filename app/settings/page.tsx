...
export default function SettingsPage() {
  ...
  const googleSheetUrlRef = useRef(googleSheetUrl);
  useEffect(() => { googleSheetUrlRef.current = googleSheetUrl; }, [googleSheetUrl]);
    ...
    try {
      ...
      if (settingsData.success) {
        ...
        if (settingsData.settings.googleSheetId) {
          setGoogleSheetUrl(
            `https://docs.google.com/spreadsheets/d/${settingsData.settings.googleSheetId}/edit`
          );
        } else {
          setGoogleSheetUrl("");
        }
  ...
  useEffect(() => {
    ...
    if (googleSuccess === "true") {
      setTimeout(() => {
        fetchSettings().then(() => {
          const url = googleSheetUrlRef.current;
          if (url) {
    ...
    try {
      ...
      const body = isEdit
        ? {
            ...
            name: newNumName,
            wabaId: newWabaId,
            whatsappPhoneNumberId: newPhoneId,
