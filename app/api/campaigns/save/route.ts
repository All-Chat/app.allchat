  ...
  try {
    ...
    if (!limitCheck.allowed) {
      return NextResponse.json({
        ...
        limitExceeded: true,
        limitInfo: { resource: "campaigns", currentUsage: limitCheck.currentUsage, limit: limitCheck.limit, period: limitCheck.period, remaining: limitCheck.remaining },
      }, { status: 429 });
    }
    ...
    const newCampaign = await Campaign.create({
      userId, tenantId, createdBy: userId, name: name.trim(), templateName, templateCategory, variables, mappedVariables, generateOtp, otpLength,
    ...
    if (scheduledAt) {
      await ScheduledTrigger.create({ userId, tenantId, campaignId: newCampaign._id, expireAt: new Date(scheduledAt), processed: false });
    }
